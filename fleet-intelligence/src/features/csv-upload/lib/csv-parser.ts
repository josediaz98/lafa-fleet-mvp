...
export function findClosestDriverId(
  input: number,
  activeDrivers: { didiDriverId: number }[],
): number | null {
...
export function validateRow(
  ...
  if (!exactMatch) {
    ...
    return {
      ...row,
      estado: 'error',
      errorMsg: `Driver ID no encontrado${hint}`,
    };
  }
  row.matchedDriverName =
    exactMatch.fullName ?? `Driver ${exactMatch.didiDriverId}`;
  row.matchType = 'exact';
  ...
  if (row.costo > 500 && tripMinutes < 15) {
    return {
      ...row,
      estado: 'warning',
      errorMsg: `Tarifa alta (${formatMXN(row.costo)}) para viaje de ${tripMinutes}min`,
    };
  }

  if (row.pickupLat !== undefined && row.pickupLng !== undefined) {
    if (
      row.pickupLat < CDMX_LAT_MIN ||
      row.pickupLat > CDMX_LAT_MAX ||
      row.pickupLng < CDMX_LNG_MIN ||
      row.pickupLng > CDMX_LNG_MAX
    ) {
      return {
        ...row,
        estado: 'warning',
        errorMsg: 'Coordenadas fuera de CDMX',
      };
    }
  }
  ...
  if (currentWeekStart && currentWeekEnd && parts.length === 3) {
    ...
    if (isoDate < currentWeekStart || isoDate > currentWeekEnd) {
      return {
        ...row,
        estado: 'warning',
        errorMsg: `Fecha fuera de la semana actual (${currentWeekStart} a ${currentWeekEnd})`,
      };
    }
  }
...
export function parseCsvText(text: string): ParsedRow[] {
  ...
  const inicioIdx = headers.findIndex(
    (h) => h.includes('inicio') || h === 'initial time',
  );
  const finIdx = headers.findIndex(
    (h) => h.includes('fin') || h === 'final time',
  );
  const costoIdx = headers.findIndex(
    (h) => h.includes('costo') || h === 'cost',
  );
  const propinaIdx = headers.findIndex(
    (h) => h.includes('propina') || h === 'tip',
  );
  const latIdx = headers.findIndex((h) => h.includes('lat'));
  const lngIdx = headers.findIndex(
    (h) => h.includes('lng') || h.includes('lon'),
  );
  const initialCoordsIdx = headers.findIndex(
    (h) => h === 'initial coordinates',
  );
  ...
  const requiredHeaders: [number, string][] = [
    ...
    [fechaIdx, 'Date/Fecha'],
    [tripIdx, 'Trip ID'],
    [inicioIdx, 'Initial time/Inicio'],
    [finIdx, 'Final time/Fin'],
    [costoIdx, 'Cost/Costo'],
  ];
    ...
    return [
      {
        ...
        errorMsg: `Columnas requeridas no encontradas: ${missingHeaders.join(', ')}`,
      },
    ];
  }
