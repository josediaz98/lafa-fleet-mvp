...
import { addToDriversMap, removeFromDriversMap, updateDriverStatusInMap } from '@/lib/mappers';
...
export async function actionAddDriver(
  ...
  await withOptimistic(ctx, {
    optimistic: () => { ctx.dispatch({ type: 'ADD_DRIVER', payload: driver }); addToDriversMap(driver); },
    persist: () => persistNewDriver(driver),
    rollback: () => { ctx.dispatch({ type: 'REMOVE_DRIVER', payload: driver.id }); removeFromDriversMap(driver.id); },
...
export async function actionUpdateDriver(
  ...
  await withOptimistic(ctx, {
    optimistic: () => { ctx.dispatch({ type: 'UPDATE_DRIVER', payload: driver }); addToDriversMap(driver); },
    persist: () => persistUpdateDriver(driver),
    rollback: () => { ctx.dispatch({ type: 'UPDATE_DRIVER', payload: oldDriver }); addToDriversMap(oldDriver); },
...
export async function actionDeactivateDriver(
  ...
  await withOptimistic(ctx, {
    optimistic: () => { ctx.dispatch({ type: 'DEACTIVATE_DRIVER', payload: driverId }); updateDriverStatusInMap(driverId, 'inactivo'); },
    persist: () => persistDeactivateDriver(driverId),
    rollback: () => { ctx.dispatch({ type: 'REACTIVATE_DRIVER', payload: driverId }); updateDriverStatusInMap(driverId, 'activo'); },
