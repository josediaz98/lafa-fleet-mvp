import type { PayrollRecord, Trip } from '../../context/AppContext';
...
export default function PayrollDetailPanel({ record, trips }: PayrollDetailPanelProps) {
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div>
          <p className="text-xs text-lafa-text-secondary">Centro</p>
          <p className="text-sm font-medium text-lafa-text-primary">{record.center}</p>
        </div>
        <div>
          <p className="text-xs text-lafa-text-secondary">Horas trabajadas</p>
          <p className="text-sm font-medium text-lafa-text-primary">{record.hoursWorked}h</p>
        </div>
        <div>
          <p className="text-xs text-lafa-text-secondary">{'Facturaci\u00f3n'}</p>
          <p className="text-sm font-medium text-lafa-text-primary">{formatMXN(record.totalBilled)}</p>
        </div>
        <div>
      ...
      <div className="border-t border-lafa-border pt-4">
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-lafa-text-primary">{formatMXN(record.baseSalary)}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-lafa-text-secondary">Bono productividad</span>
            <span className="text-lafa-text-primary">{formatMXN(record.productivityBonus)}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-lafa-text-secondary">Overtime</span>
            <span className="text-lafa-text-primary">{formatMXN(record.overtimePay)}</span>
          </div>
          {!record.goalMet && (
            <div className="flex justify-between text-sm">
          ...
          <div className="flex justify-between text-sm font-semibold border-t border-lafa-border pt-2">
            ...
            <span className="text-lafa-accent">{formatMXN(record.totalPay)}</span>
          </div>
        </div>
        {record.totalPay > 0 && (() => {
      ...
      {trips.length > 0 && (
        <div className="border-t border-lafa-border pt-4">
          <h4 className="text-sm font-semibold text-lafa-text-primary mb-3">Viajes ({trips.length})</h4>
